...
class App:
    ...
    def create_widgets(self):
        ...
        self.instructions_label.grid(row=0, column=0, rowspan=4, padx=10, pady=5, sticky="nsew")

        # File Upload Button
        self.upload_button = tk.Button(self.root, text="Upload File", command=self.upload_file)
        self.upload_button.grid(row=0, column=1, columnspan=2, padx=10, pady=5)

        # File Path Label
        self.file_path_label = tk.Label(self.root, textvariable=self.file_path)
        self.file_path_label.grid(row=1, column=1, columnspan=2, padx=10, pady=5, sticky="w")

        # Input Textbox
        self.input_label = tk.Label(self.root, text="Input:")
        self.input_label.grid(row=2, column=1, padx=10, pady=5, sticky="w")
        self.input_textbox = tk.Text(self.root, height=10, width=50)
        self.input_textbox.grid(row=3, column=1, padx=10, pady=5, sticky="nsew")

        # Output Textbox
        self.output_label = tk.Label(self.root, text="Output:")
        self.output_label.grid(row=4, column=1, padx=10, pady=5, sticky="w")
        self.output_textbox = tk.Text(self.root, height=10, width=50)
        self.output_textbox.grid(row=5, column=1, padx=10, pady=5, sticky="nsew")

        # Execute Button
        self.execute_button = tk.Button(self.root, text="Execute Program", command=self.execute_program)
        self.execute_button.grid(row=6, column=1, columnspan=2, padx=10, pady=5)

        # Configure resizing behavior
        self.root.columnconfigure(0, weight=1)
        self.root.columnconfigure(1, weight=3)
        self.root.rowconfigure([3, 5], weight=1)
